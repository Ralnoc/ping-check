...
def quiet_ping(dest_addr, timeout = 2, count = 4, psize = 64):
    ...
    if plist:
        mrtt = max(filter(None, plist))
        artt = sum(filter(None, plist)) / len(filter(None, plist))

    return percent_lost, mrtt, artt


def detailed_ping(dest_addr, timeout = 2, count = 4, psize = 64):
    """
    Send `count' ping with `psize' size to `dest_addr' with
    the given `timeout' and display the result.
    Returns `percent' lost packages, 'successful' packets sent, 'lost' packets, 
    `max' round trip time, 'min' round trip time and `avrg' round trip time.
    """
    maxrtt = None
    minrtt = None
    artt = None
    ...
    # Find lost package percent
    percent_lost = 100 - (len(filter(None,plist)) * 100 / count)
    csucc = len(filter(None, plist))
    cfail = len(filter(is_none, plist))

    # Find max and avg round trip time
    if plist:
        maxrtt = max(filter(None, plist))
        minrtt = min(filter(None, plist))
        artt = sum(filter(None, plist)) / len(filter(None, plist))
